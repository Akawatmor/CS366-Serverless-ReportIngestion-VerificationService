...
resource "aws_api_gateway_method" "get_audit" {
  rest_api_id   = aws_api_gateway_rest_api.api.id
  resource_id   = aws_api_gateway_resource.reports_audit.id
  http_method   = "GET"
  authorization = "NONE"
}
...
# /v1/reports/events
# ==========================
resource "aws_api_gateway_resource" "reports_events" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  parent_id   = aws_api_gateway_resource.reports.id
  path_part   = "events"
}
...
resource "aws_api_gateway_method" "get_events" {
  rest_api_id   = aws_api_gateway_rest_api.api.id
  resource_id   = aws_api_gateway_resource.reports_events.id
  http_method   = "GET"
  authorization = "NONE"
}
...
# ==========================
resource "aws_api_gateway_resource" "reports_upload_url" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  parent_id   = aws_api_gateway_resource.reports.id
...
resource "aws_api_gateway_deployment" "api" {
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_resource.reports_audit.id,
      aws_api_gateway_resource.reports_events.id,
      ...
      aws_api_gateway_method.get_audit.id,
